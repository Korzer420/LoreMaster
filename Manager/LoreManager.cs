...
internal class LoreManager
{
    ...
    public void PreventReading(PlayMakerFSM self)
    {
        if (CanRead)
            return;
        if (string.Equals(self.FsmName, "Inspection") && PowerManager.GetPowerByKey(self.FsmVariables.FindFsmString("Convo Name")?.Value, out Power power, false))
            self.GetState("Init").ClearTransitions();
        // Prevent the player from reading lore tablets without the item (rando only)
        else if (string.Equals(self.FsmName, "inspect_region") && (PowerManager.GetPowerByKey(self.FsmVariables.FindFsmString("Game Text Convo")?.Value, out power, false)
