    ...
    private void CheckIfZoteSpawn(PlayMakerFSM fsm)
    {
        fsm.AddState(new HutongGames.PlayMaker.FsmState(fsm.Fsm)
        {
            ...
            Actions = new HutongGames.PlayMaker.FsmStateAction[]
            {
                new Lambda(() => 
                {
                    if(LoreManager.Instance.Traveller[Enums.Traveller.Zote].CurrentStage >= LoreManager.Instance.Traveller[Enums.Traveller.Zote].Locations.IndexOf("Zote Deepnest")
